...
suite "Conjugate Gradient":
  test "Example 1":
    ...
    check allClose(u, @[1.0/11.0, 7.0/11.0])
  ...
  test "Example 2":
    ...
    let u = conjugate_gradient(A, b)
    check allClose(u, @[1.0, 0.5, 1.0/3.0, 0.25])
...
suite "Incomplete LU":
  test "Example 1":
    ...
    check reference_ilu.ia == computed_ilu.ia
    check reference_ilu.ja == computed_ilu.ja
    check allClose(reference_ilu.aa, computed_ilu.aa)
  ...
  test "ILU solve":
    ...
    check allClose(x, @[5.0, 0.5, -1.0, 1.0])

suite "Preconditioned CG":
  test "Example 1":
    var A: SparseMatrix
    A.ia = @[0, 2, 4]
    A.ja = @[0, 1, 0, 1]
